...
public class PermissionsService {
    ...
    public Map<JsonPointer, Set<Permission>> merge(List<Map<JsonPointer, Set<Permission>>> permissions) {
        ...
        return mergedAttributePermissions.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, entry -> entry.getValue().getCombinedPermissions()));
    }

    // TreeMap is used for a reason, concurrent map would not help
    // There is no good way to achieve the same in functional style
    /**
    ...
    private Map<JsonPointer, Merge> combinePermissionByAttribute(List<Map<JsonPointer, Set<Permission>>> permissions) {
        ...
        for (int i = 0; i < permissions.size(); i++) {
            ...
            permissionsPerRole.forEach((attribute, attributePermissionsPerRole) -> mergedPermissions.computeIfAbsent(attribute, value -> new Merge()).permissions.put(sourceIndex, attributePermissionsPerRole));
        }
        return mergedPermissions;
    }

    // Refactoring would not help much in that case
    @SuppressWarnings("PMD.AvoidDeeplyNestedIfStmts")
    private void propagateParentPermissionsToClosestChild(Map<JsonPointer, Merge> attributePermissions, int numberOfSources) {
        ...
        attributePermissions.forEach((attribute, attributeMerge) -> {
            if (attributeMerge.permissions.size() != numberOfSources) {
                log.debug(">> Attribute \'{}\' is not fully merged - it only has {} from {} sources", attribute, attributeMerge.getCombinedPermissions(), attributeMerge.getSourceIndexes());
                JsonPointer head = attribute.head();
                while (head != null) {
                    if (attributePermissions.containsKey(head)) {
                        Merge parentMerge = attributePermissions.get(head);
                        Map<Integer, Set<Permission>> missingPermissions = findMissingPermissions(parentMerge.permissions, attributeMerge.getSourceIndexes());
                        if (!missingPermissions.isEmpty()) {
                            log.debug(">>> Applying permissions {} from attribute \'{}\' to attribute \'{}\'", missingPermissions, head, attribute);
     ...
     */
    private Map<Integer, Set<Permission>> findMissingPermissions(Map<Integer, Set<Permission>> permissions, Set<Integer> blacklistedSourceIndexes) {
        return permissions.entrySet().stream().filter(entry -> {
    ...
    private class Merge {
        ...
        public boolean equals(final Object o) {
            ...
            if (!(o instanceof PermissionsService.Merge)) return false;
            final PermissionsService.Merge other = (PermissionsService.Merge) o;
            if (!other.canEqual((Object) this)) return false;
            final Object this$permissions = this.permissions;
            final Object other$permissions = other.permissions;
            if (this$permissions == null ? other$permissions != null : !this$permissions.equals(other$permissions)) return false;
            return true;
        }

        protected boolean canEqual(final Object other) {
            return other instanceof PermissionsService.Merge;
        }
        ...
        public int hashCode() {
            final int PRIME = 59;
            int result = 1;
            final Object $permissions = this.permissions;
        ...
        public String toString() {
            return "PermissionsService.Merge(permissions=" + this.permissions + ")";
        }
    }
}
