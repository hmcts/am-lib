...
public class FilterService {
    ...
    private List<JsonPointer> filterPointersWithReadPermission(Map<JsonPointer, Set<Permission>> attributePermissions) {
        return attributePermissions.entrySet().stream().filter(entry -> entry.getValue().contains(READ)).map(Map.Entry::getKey).collect(Collectors.toList());
    }

    private List<JsonPointer> reducePointersToUniqueList(List<JsonPointer> nodesWithRead) {
        return nodesWithRead.stream().sorted(Comparator.comparing(JsonPointer::toString)).reduce(new ArrayList<>(), (List<JsonPointer> result, JsonPointer pointerCandidate) -> {
            // already contains parent so no point adding
            if (result.stream().noneMatch(acceptedPointer -> pointerCandidate.toString().startsWith(acceptedPointer.toString() + "/"))) {
    ...
    // Sorted map is needed; instance is local as well
        ...
        pointersByDepth.forEach(map -> map.forEach((key, value) -> {
            ...
            if (node instanceof ObjectNode) {
                ...
                if (filteredNode.size() == 0 && key.head() != null) {
                    ((ObjectNode) resource.at(key.head())).remove(key.last().toString().substring(1));
                }
            }
        }));
    }

    // Objects cannot be created outside the loop
    /**
    ...
    private Map<Integer, Map<JsonPointer, Set<String>>> decomposePointersByDepth(List<JsonPointer> nodes) {
        return nodes.stream().reduce(new TreeMap<>(Collections.reverseOrder()), (Map<Integer, Map<JsonPointer, Set<String>>> result, JsonPointer pointer) -> {
    ...
    private void removeFieldsWithoutReadPermission(JsonNode resource, List<JsonPointer> nodesWithRead, List<JsonPointer> nodesWithoutRead) {
        nodesWithoutRead.forEach(pointerCandidateForRemoval -> {
            ...
            List<JsonPointer> childPointersWithRead = nodesWithRead.stream().filter(pointerWithRead -> pointerWithRead.toString().startsWith(pointerCandidateForRemoval.toString() + "/")).collect(Collectors.toList());
